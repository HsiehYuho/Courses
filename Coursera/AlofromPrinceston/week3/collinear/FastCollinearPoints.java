...
public class FastCollinearPoints{
	...
	public LineSegment[] segments(){
		...
		for (int i = 0; i < setOfPoints.length-3;i++){
			...
			Arrays.sort(setOfPoints);
			// sort the set of points by slope order of i, nothing to do with p
			Arrays.sort(setOfPoints,setOfPoints[i].slopeOrder()); 
			// find the last collinear to p point 	
			// take p as origin
			for (int p = 0, first = 1, last = 2; last < setOfPoints.length; last++){
				...
				// last is not the very last point, but the last-1
				...
				first = last; // so move the first to last instead of last - 1
			}
		}
